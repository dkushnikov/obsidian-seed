# Session Workflow — Working with an Agent in Your Vault, Day to Day
...
You open a terminal in your vault directory, run `claude` or `codex`, and wait for the agent to show its session brief.

The agent reads:
- `CLAUDE.md` or `AGENTS.md` — vault conventions and structure
- `_claude/MEMORY.md` — portable vault memory with persistent decisions and context
...
**Your job in the open phase:** read the brief. Decide what today is about. Say it out loud to the agent.
...
- **Let the agent push back.** If it asks a clarifying question, answer honestly. If it disagrees, consider the disagreement before overriding it.
...
2. **Update `_claude/MEMORY.md`** — capture any decisions, preferences, or patterns that future sessions should know about. Not every small thing. Just the "if you forgot this, the next session would be worse" stuff.
...
**Why closing matters more than opening:** the open phase is the agent catching up to you. The close phase is you handing context forward to the next session. If you skip the close, the next session is stuck with stale context — and compound interest breaks.
...
If you use the vault once every two weeks, the vault has two weeks of new context stuck in your head that it doesn't know about. When you come back and try to do something complex — say, ask the agent to help with a major decision — the agent is working with stale data and your current head is full of fresh data that the vault hasn't seen.

The failure mode is silent: the agent produces plausible-looking output that is subtly wrong because its input was thin. You don't notice until later.
...
Example. You've been away from the vault for ten days. You want the agent to help you think through a big decision about your team structure. Instead of opening a session and asking "help me think through team structure," do this first:
...
No agent session required. This is you talking to your future self — and to your future agent sessions — without the overhead of a full conversation.

The value: next time you do open a session with a real task, the vault is not as stale, and the agent has context it wouldn't otherwise have.

### Reality-check against memory before deep operations

One more pattern for episodic users: **check the agent's memory before trusting it**.

The agent reads `_claude/MEMORY.md` at session start. But `_claude/MEMORY.md` may be out of date — decisions you made two weeks ago in your head, but never wrote down. Before a serious operation, ask:

> "What do you know about [topic]? Summarize from _claude/MEMORY.md and recent session logs."

If the summary is stale, correct it before proceeding:

> "That's out of date. Since then, X happened and I decided Y. Update _claude/MEMORY.md with that."

Now the agent's memory matches reality. The next complex operation runs on accurate context.
...
- You're about to ask the agent to do something that will take 15+ minutes of its attention
- You're about to make a decision that matters
- You're about to run a multi-step process that depends on earlier steps
- You're going to ask the agent to generate substantial content based on your situation
...
2. **Explicitly name the operation** to the agent: "This is going to be a strategic planning session about X. Before we start, I want to make sure you have the context you need."
3. **Ask the agent what it knows and what it's missing**: "What do you know about X? What would you want to know that you don't see in the vault?"
4. **Fill in the gaps the agent names** before starting the real work
...
## When it feels like the agent forgot everything

You come back after some time away. You open a session. The agent seems generic, like the vault doesn't exist. This is the single most common "this tool is broken" feeling.

**First, check these in order:**

1. **Did the session onboarding rule actually run?** The agent should have read `CLAUDE.md` or `AGENTS.md`, `_claude/MEMORY.md`, recent session logs, and TODO at session start. Ask: "What did you read at session start? Summarize." If the answer is thin, the onboarding rule didn't fire — or it's not pointed at the right files.
2. **Is `_claude/MEMORY.md` too long?** If `_claude/MEMORY.md` is over 200 lines, the agent may be skipping or summarizing. Prune or restructure.
3. **Is there a recent session log at all?** If you stopped writing session logs a month ago, the agent has no recent context. The vault exists but the continuity layer is broken.
4. **Are there stale contradictions in memory?** If `_claude/MEMORY.md` says "we decided X" but you've since changed your mind, the agent is working with stale data. Fix the memory.

**If all four are fine and the agent still feels generic:** the issue is usually the current session's ask, not the agent's context. Vague questions get vague answers even with perfect memory. Try again with a more specific framing.
...
- **Session 1:** You build the vault. The agent is a smart stranger who just read your Me.md.
- **Session 2-3:** Modest compound. The agent remembers your first session's decisions. Quality is maybe 10% better.
- **Session 5-10:** Noticeable compound. The agent starts connecting dots across sessions. You start experiencing "oh, it remembered that" moments.
- **Session 20-30:** Strong compound. The agent has a working model of how you think. Its pushback starts being useful. Its suggestions start being non-obvious.
...
**Batching all your asks into one giant session prompt.** Three focused asks produce better output than one mega-ask. The agent responds to each, but the output is diluted.
...
**Treating `_claude/MEMORY.md` as a diary.** `_claude/MEMORY.md` is short, high-signal decisions and context. If it's becoming a diary, put the diary in `Reflections/` and keep `_claude/MEMORY.md` lean.
...
**Asking the agent to "organize my notes."** Too vague. Specific: "I have 12 notes in `Reflections/` that should probably be tagged differently. Here's what I think the tags should be. Let me know if that's off." That an agent can do.

**Trusting stale memory.** See the calibration pass above. Before serious operations, check the agent's context is current.
...
- `CLAUDE.md` or `AGENTS.md` at vault root — vault orientation for the selected agent
...
- `.claude/rules/session-onboarding.md` and `.claude/rules/session-offboarding.md` for Claude Code, or equivalent `## Session Start` and `## Session Close` sections in `AGENTS.md` for Codex
